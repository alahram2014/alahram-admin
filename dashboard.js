...
const UNIT_SCOPE_LABELS = {
  carton: 'كرتون',
  pack: 'دستة',
};

const UNIT_SCOPE_PRESETS = [
...
function toast(message, level = 'info') {
  ...
  if (!el) {
    ...
    el.className = 'toast smart-toast info';
    document.body.appendChild(el);
  }
  el.className = `toast smart-toast ${level}`;
...
function appSettingValue(key) {
  const row = safeRows(state.appSettings).find((item) => String(item.key || '') === String(key));
  return row?.value ?? null;
}

function tierScopeKey(tierName) {
  return `tier_scope:${tierName}`;
}
...
function normalizeTierScope(value) {
  const parsed = typeof value === 'string' ? parseJSONSafe(value, null) : value;
  const carton = parsed?.carton !== false;
  const pack = parsed?.pack !== false;
  return { carton, pack };
}

function tierScopeFor(tierName) {
  return normalizeTierScope(appSettingValue(tierScopeKey(tierName)));
}

function tierScopeText(tierName) {
  const scope = tierScopeFor(tierName);
  const parts = [];
  if (scope.carton) parts.push('كرتون');
  if (scope.pack) parts.push('دستة');
  return parts.length ? parts.join(' + ') : 'لا يطبق';
}

function tierScopeBadge(tierName) {
  return `<span class="status orange">${escapeHtml(tierScopeText(tierName))}</span>`;
}
...
function buildSearchCandidates(query) {
  ...
  (state.orders || []).forEach((o) => {
    ...
    candidates.push({
      type: 'order-detail',
      id: o.id,
      label: `فاتورة ${o.invoice_number || o.order_number || o.id}`,
      meta: `${customer.name || '—'} • ${rep.name || 'مباشر'} • ${statusLabel(o.workflow_status || o.status)}`,
      score: fuzzyScore(`${o.invoice_number} ${o.order_number} ${o.id} ${customer.name} ${customer.phone} ${rep.name} ${rep.phone} ${o.tier_name} ${o.status} ${o.workflow_status}`, q),
    });
  });
  (state.customers || []).forEach((c) => candidates.push({
    type: 'customer-detail',
    id: c.id,
    label: c.name,
    meta: `${c.phone || '—'} • ${c.customer_type === 'rep' ? 'مندوب' : 'مباشر'}`,
    score: fuzzyScore(`${c.name} ${c.phone} ${c.address} ${c.location} ${c.username} ${c.customer_type}`, q),
  }));
  (state.reps || []).forEach((r) => candidates.push({
    type: 'rep-detail',
    id: r.id,
    label: r.name,
    meta: `${r.phone || '—'} • ${r.region || '—'}`,
    score: fuzzyScore(`${r.name} ${r.phone} ${r.region} ${r.username}`, q),
  }));
  (state.products || []).forEach((p) => candidates.push({
    type: 'product-detail',
    id: p.product_id,
    label: p.product_name,
    meta: `${companyById(p.company_id)?.company_name || '—'} • ${statusLabel(p.status)}`,
    score: fuzzyScore(`${p.product_name} ${p.product_id} ${companyById(p.company_id)?.company_name || ''} ${statusLabel(p.status)}`, q),
  }));
  (state.companies || []).forEach((c) => candidates.push({
    type: 'company-detail',
    id: c.company_id,
    label: c.company_name,
    meta: c.company_id,
    score: fuzzyScore(`${c.company_name} ${c.company_id}`, q),
  }));
  (state.tiers || []).forEach((t) => candidates.push({
    type: 'tier-detail',
    id: t.tier_name,
    label: t.visible_label || t.tier_name,
    meta: `${tierScopeText(t.tier_name)} • ${num(t.discount_percent || 0)}%`,
    score: fuzzyScore(`${t.tier_name} ${t.visible_label} ${tierScopeText(t.tier_name)}`, q),
  }));
  candidates.sort((a, b) => b.score - a.score);
  return candidates.filter((c) => c.score > 0).slice(0, 8);
}

function renderSearchResultsPanel(query) {
  const items = buildSearchCandidates(query);
  if (!String(query || '').trim()) {
    return '<div class="empty-state">اكتب للبحث في كل الداتا. النتائج تظهر هنا مباشرة.</div>';
  }
  if (!items.length) {
    return '<div class="empty-state">لا توجد نتائج مطابقة. جرّب كلمات أقل أو أضف مسافة بين الكلمات.</div>';
  }
  return items.map((item) => `
    <div class="search-hit">
      <div class="search-hit__top">
        <div>
          <div class="search-hit__title">${escapeHtml(item.label || '—')}</div>
          <div class="search-hit__meta">${escapeHtml(PAGE_META[item.type] || item.type || '—')} • ${escapeHtml(item.meta || '—')}</div>
        </div>
        <span class="status orange">${escapeHtml(item.type === 'order-detail' ? 'فاتورة' : item.type === 'customer-detail' ? 'عميل' : item.type === 'rep-detail' ? 'مندوب' : item.type === 'company-detail' ? 'شركة' : item.type === 'product-detail' ? 'منتج' : 'شريحة')}</span>
      </div>
      <div class="search-hit__actions">
        <button class="btn primary" data-action="search-open" data-type="${escapeHtml(item.type)}" data-id="${escapeHtml(item.id)}" type="button">فتح</button>
        <button class="btn" data-action="search-edit" data-type="${escapeHtml(item.type)}" data-id="${escapeHtml(item.id)}" type="button">تعديل مباشر</button>
      </div>
    </div>
  `).join('');
}
...
function updateGlobalSearchResults(query) {
  ...
  if (panel) panel.innerHTML = renderSearchResultsPanel(query);
}
...
function renderDashboardHome() {
  ...
  const sections = {
    ...
    tiers: modulePanel('الشرائح', 'التحكم في التسعير والخصم والوحدات', [
      hubActionCard('كل الشرائح', 'قائمة الشرائح الحالية', 'go-tiers', '💠'),
      hubActionCard('وحدات الخصم', 'اختيار كرتون أو دستة أو الاثنين', 'go-tiers', '🧩'),
    ...
    <section class="card hero-card">
      <div class="hero-top">
        ...
        <div class="hero-search">
          ...
          <div id="globalSearchResults" class="search-results-panel">${renderSearchResultsPanel(state.searchText)}</div>
          ${searchSummary ? `<div class="search-result"><div class="small">أفضل نتيجة مطابقة</div><div class="search-result__title">${escapeHtml(searchSummary.label || '—')}</div><div class="search-result__meta">${escapeHtml(PAGE_META[searchSummary.type] || searchSummary.type || '—')}</div></div>` : ''}
        </div>
      </div>
  ...
  return `
    ...
    <div class="list">${rows.length ? rows.map((tier) => {
      ...
      return listItem(tier.visible_label || tier.tier_name || '—', `
        ...
        الوحدات: ${escapeHtml(tierScopeText(tier.tier_name))}<br/>
...
function tierFormHtml(tier) {
  ...
  const scope = tierScopeFor(tier?.tier_name || '');
  return `
    <div class="grid cols-2">
      ...
      <label class="field"><span>الوحدات التي يطبق عليها الخصم</span>
        <div class="choice-grid">
          <label class="check-chip"><input id="${prefix}TierScopeCarton" type="checkbox" ${scope.carton ? 'checked' : ''} /> كرتون</label>
          <label class="check-chip"><input id="${prefix}TierScopePack" type="checkbox" ${scope.pack ? 'checked' : ''} /> دستة</label>
        </div>
      </label>
    ...
    <div class="grid cols-2">
      ...
      <div class="card">
        ...
        <div class="grid cols-2">
          ...
          <div class="item compact"><div class="item-title">الوحدات</div><div class="item-meta">${escapeHtml(tierScopeText(tier.tier_name))}</div></div>
    ...
    <div class="topbar-shell">
      <div class="topbar">
        <div class="topbar-brand">
          ...
          <div>
            <h1>لوحة التحكم</h1>
            <div class="small">موبايل أول • ربط مباشر بقاعدة البيانات • بحث حيّ</div>
          </div>
        </div>
...
function rememberInputState(target) {
...
function restoreInputState() {
  const info = state.focusedInput;
  if (!info?.id) return;
  const el = document.getElementById(info.id);
  if (!el) return;
  try {
    el.focus({ preventScroll: true });
    if (typeof info.start === 'number' && typeof info.end === 'number' && typeof el.setSelectionRange === 'function') {
...
function bindShell() {
  ...
  if (search) {
    const runSearch = () => {
      ...
      updateGlobalSearchResults(search.value);
    };
    ...
    search.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        ...
        state.selected = {
          ...
          tierName: result.type === 'tier-detail' ? result.id : state.selected.tierName,
...
function bindPageEvents() {
  const onClick = async (event) => {
    ...
    if (action === 'search-open' || action === 'search-edit') {
      const type = target.getAttribute('data-type');
      const id = target.getAttribute('data-id');
      if (type === 'order-detail') { state.selected.orderId = id; navigate('order-detail'); return; }
      if (type === 'customer-detail') { state.selected.customerId = id; navigate('customer-detail'); return; }
      if (type === 'rep-detail') { state.selected.repId = id; navigate('rep-detail'); return; }
      if (type === 'company-detail') { state.selected.companyId = id; navigate('company-detail'); return; }
      if (type === 'product-detail') { state.selected.productId = id; navigate('product-detail'); return; }
      if (type === 'tier-detail') { state.selected.tierName = id; navigate('tier-detail'); return; }
    }
    try {
  ...
  const onInput = (event) => {
    const target = event.target;
    if (target.id === 'orderSearch') { state.filters.q = target.value; rememberInputState(target); clearTimeout(bindPageEvents._searchTimer); bindPageEvents._searchTimer = setTimeout(render, 180); }
    if (target.id === 'customerSearch') { state.filters.q = target.value; rememberInputState(target); clearTimeout(bindPageEvents._searchTimer); bindPageEvents._searchTimer = setTimeout(render, 180); }
    if (target.id === 'productSearch') { state.filters.q = target.value; rememberInputState(target); clearTimeout(bindPageEvents._searchTimer); bindPageEvents._searchTimer = setTimeout(render, 180); }
  };
...
async function loadAll() {
  ...
  try {
    const results = await Promise.allSettled([
      ...
      apiGet('app_settings', { select: 'key,value,visible,updated_at', order: 'key.asc' }),
    ...
    state.tiers = v(8);
    state.appSettings = v(9);
    state.dailyDeals = v(10);
    state.flashOffers = v(11);
    state.customerAssignments = v(12);
    state.views = {
      ordersStatus: v(13),
      topCompanies: v(14),
      topProducts: v(15),
      topCustomers: v(16),
      repsPerformance: v(17),
      salesDaily: v(18),
      repSales: v(19),
    };
    ...
    state.appSettings = (state.appSettings || []).map((s) => ({
      ...s,
...
async function saveTierScope(tierName, scope) {
  if (!tierName) return;
  const key = tierScopeKey(tierName);
  const value = JSON.stringify(normalizeTierScope(scope));
  const existing = safeRows(state.appSettings).find((row) => String(row.key || '') === String(key));
  if (existing) {
    await apiMut('PATCH', 'app_settings', { value, visible: true, updated_at: new Date().toISOString() }, { key: `eq.${key}` });
  } else {
    await apiMut('POST', 'app_settings', { key, value, visible: true, updated_at: new Date().toISOString() });
  }
}
...
async function saveTier(id) {
  ...
  const scope = {
    carton: !!document.getElementById(id ? 'editTierScopeCarton' : 'TierScopeCarton')?.checked,
    pack: !!document.getElementById(id ? 'editTierScopePack' : 'TierScopePack')?.checked,
  };
  ...
  if (id && id !== body.tier_name) {
    ...
    await apiMut('DELETE', 'app_settings', {}, { key: `eq.${tierScopeKey(id)}` }).catch(() => {});
  } else if (id) {
    await apiMut('PATCH', 'tiers', body, { tier_name: `eq.${id}` });
  } else {
  ...
  await saveTierScope(body.tier_name || id, scope);
